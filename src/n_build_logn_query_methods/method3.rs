...
impl Method3 {
    pub fn new(level: &[usize], pre_order: &[usize], time_in: &[usize]) -> Self {
        ...
        let mut level_size = (1 << n.ilog2()) as usize;
        let mut prev = n;

        while level_size >= 1 {
            let mid = 2 * level_size;
            let (left, right) = seg_tree.split_at_mut(mid);
            left[level_size..prev]
                .par_iter_mut()
                .enumerate()
                .for_each(|(idx, val)| {
                    let i = level_size + idx;
                    *val = std::cmp::min(right[2 * i - mid], right[2 * i + 1 - mid]);
                });
